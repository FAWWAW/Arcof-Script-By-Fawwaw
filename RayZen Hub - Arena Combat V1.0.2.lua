...
getgenv().FgzConfig = {
    ...
    Icon = "⚔️",
    MainIcon = "rbxthumb://type=Asset&id=119144536888000&w=150&h=150",     
    MovementIcon = "👣",                      
    VisualsIcon = "👀",                       
...
-- Setup Pijakan Anti Void
if getgenv().AntiVoidPart then getgenv().AntiVoidPart:Destroy() end
local antiVoidPart = Instance.new("Part")
antiVoidPart.Size = Vector3.new(20, 1, 20)
antiVoidPart.Transparency = 1
antiVoidPart.Anchored = true
antiVoidPart.CanCollide = true
antiVoidPart.Name = "RayZenAntiVoid"
antiVoidPart.Parent = workspace
getgenv().AntiVoidPart = antiVoidPart
...
createToggle("Anti Void (Gak Nembus Lantai)", MainTab, function(state) getgenv().AntiVoidEnabled = state end)
...
-- LOGIKA BARU V1.0.2: Auto-Click "MAIN" Saat Mati
task.spawn(function()
    while task.wait(1) do
        if getgenv().AutoClickMainEnabled then
            -- Cek kalau darah 0 atau mati
...
table.insert(ActiveConnections, RunService.Stepped:Connect(function()
    if getgenv().NoclipEnabled and LocalPlayer.Character then
        for _, v in pairs(LocalPlayer.Character:GetDescendants()) do
            -- Kita tidak noclip "HumanoidRootPart" jika Anti Void menyala dan kita di udara, agar tidak loss ke tanah
...
table.insert(ActiveConnections, RunService.RenderStepped:Connect(function()
    ...
    -- LOGIKA BARU V1.0.2: Anti Void (Noclip Aman)
    local avPart = getgenv().AntiVoidPart
    if getgenv().AntiVoidEnabled and root and avPart then
        avPart.CFrame = root.CFrame * CFrame.new(0, -3.8, 0)
    elseif avPart then
        avPart.CFrame = CFrame.new(0, 999999, 0) -- Jauhkan dari map jika off
    end

    -- LOGIKA BARU V1.0.2: No CD Reload (Bypass Cooldown)
        ...
        if tool then
            for _, v in pairs(tool:GetDescendants()) do
                if v:IsA("NumberValue") or v:IsA("IntValue") then
                    ...
                    -- Set waktu reload/cooldown ke 0
                    if string.match(nameLow, "reload") or string.match(nameLow, "cooldown") or string.match(nameLow, "wait") or string.match(nameLow, "delay") then
                        v.Value = 0
                    -- Spoof ammo jika sistem pelurunya pakai value sederhana
                    elseif string.match(nameLow, "ammo") or string.match(nameLow, "clip") then
                        if v.Value < 10 then v.Value = 999 end 
                    end
                end
            end
        end
