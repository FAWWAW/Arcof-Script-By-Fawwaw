-- =========================================================
-- RAYZEN HUB - UNIVERSAL SCRIPT
-- Made By Fawwawzx - V1.0.3
-- =========================================================
...
-- Konfigurasi Ikon
getgenv().FgzConfig = {
    Title = "RayZen Hub - Universal Script", 
    Subtitle = "Made By Fawwawzx - V1.0.2",
...
getgenv().AutoVoteEnabled = false
getgenv().AutoClickMainEnabled = false
getgenv().AntiVoidEnabled = false
getgenv().NoCdReloadEnabled = false
getgenv().SelectedWeaponType = "Pistol/Gun"
...
if getgenv().AntiVoidPart then getgenv().AntiVoidPart:Destroy(); getgenv().AntiVoidPart = nil end
...
local TopIcon
if string.find(getgenv().FgzConfig.Icon, "rbxthumb") or string.find(getgenv().FgzConfig.Icon, "rbxassetid") then
    TopIcon = Instance.new("ImageLabel", Header)
    TopIcon.Image = getgenv().FgzConfig.Icon
else
    TopIcon = Instance.new("TextLabel", Header)
...
local FloatingLogo
if string.find(getgenv().FgzConfig.Icon, "rbxthumb") or string.find(getgenv().FgzConfig.Icon, "rbxassetid") then
    FloatingLogo = Instance.new("ImageButton", ScreenGui)
    FloatingLogo.Image = getgenv().FgzConfig.Icon
else
    FloatingLogo = Instance.new("TextButton", ScreenGui)
...
CloseBtn.MouseButton1Click:Connect(function()
    ...
    getgenv().AutoAimEnabled = false; getgenv().AutoShotEnabled = false; getgenv().AutoRunEnabled = false; getgenv().AutoFarmEnabled = false
    getgenv().HitboxExpanderEnabled = false; getgenv().AutoClickMainEnabled = false; getgenv().AntiVoidEnabled = false; getgenv().NoCdReloadEnabled = false
    ...
    for _, g in pairs(EspGuis) do if g then g:Destroy() end end
...
Watermark.Text = "Made By Fawwawzx With ❤️"
...
local function createTab(name, iconID, order)
    ...
    local Icon
    if string.find(iconID, "rbxthumb") or string.find(iconID, "rbxassetid") then
        Icon = Instance.new("ImageLabel", TabBtn)
        Icon.Image = iconID
        -- Hapus ImageColor3 hitam untuk rbxthumb agar gambar berwarna aslinya muncul jika berwarna
        if not string.find(iconID, "rbxthumb") then
    ...
    TabBtn.MouseButton1Click:Connect(function()
        for tName, frame in pairs(TabFrames) do
            ...
            if TabButtons[tName].Icon:IsA("ImageLabel") and not string.find(TabButtons[tName].Icon.Image, "rbxthumb") then
                TabButtons[tName].Icon.ImageColor3 = Palette.Black
            elseif TabButtons[tName].Icon:IsA("TextLabel") then
                TabButtons[tName].Icon.TextColor3 = Palette.Black
            end
        end
        TabBtn.BackgroundColor3 = Palette.ButtonOff
        Label.TextColor3 = Palette.Button
        if Icon:IsA("ImageLabel") and not string.find(Icon.Image, "rbxthumb") then 
            Icon.ImageColor3 = Palette.Button 
        elseif Icon:IsA("TextLabel") then 
            Icon.TextColor3 = Palette.Button 
        end
    end)
    return ContentFrame
end
...
-- [[ 3. MENGISI TABS BERDASARKAN URUTAN BARU ]]
...
-- === MAIN TAB ===
createSectionHeader("Welcome to RayZen Hub", MainTab)
local InfoBG = Instance.new("Frame", MainTab)
InfoBG.Size = UDim2.new(1, -5, 0, 80)
...
InfoTxt.Text = "Universal Script V1.0.2\nDitambah fitur Anti Void, No CD Reload, & Auto Main untuk pengalaman tempur yang maksimal!"
...
createSectionHeader("Combat Tools", MainTab)

-- Weapon Selection Dropdown
local WepDropdownContainer = Instance.new("Frame", MainTab)
WepDropdownContainer.Size = UDim2.new(1, -5, 0, 35) 
WepDropdownContainer.BackgroundColor3 = Palette.Header
WepDropdownContainer.LayoutOrder = #MainTab:GetChildren()
WepDropdownContainer.ClipsDescendants = true
Instance.new("UICorner", WepDropdownContainer).CornerRadius = UDim.new(0, 8)
addStroke(WepDropdownContainer)

local WepDropdownBtn = Instance.new("TextButton", WepDropdownContainer)
WepDropdownBtn.Size = UDim2.new(1, 0, 0, 35)
WepDropdownBtn.BackgroundTransparency = 1
WepDropdownBtn.Text = "Senjata: Pistol/Gun ▼"
WepDropdownBtn.Font = Enum.Font.GothamBold
WepDropdownBtn.TextSize = 11
WepDropdownBtn.TextColor3 = Palette.Text

local WepListScroll = Instance.new("ScrollingFrame", WepDropdownContainer)
WepListScroll.Size = UDim2.new(1, 0, 0, 60)
WepListScroll.Position = UDim2.new(0, 0, 0, 35)
WepListScroll.BackgroundTransparency = 1
WepListScroll.ScrollBarThickness = 2
WepListScroll.Visible = false
WepListScroll.BorderSizePixel = 0
local WListLayout = Instance.new("UIListLayout", WepListScroll)

local weapons = {"Pistol/Gun", "Pisau/Kapak/Melee"}
for _, wepName in ipairs(weapons) do
    local wBtn = Instance.new("TextButton", WepListScroll)
    wBtn.Size = UDim2.new(1, 0, 0, 30)
    wBtn.Text = "  " .. wepName
    wBtn.TextXAlignment = Enum.TextXAlignment.Left
    wBtn.Font = Enum.Font.GothamMedium
    wBtn.TextSize = 11
    wBtn.BackgroundColor3 = Palette.Header
    wBtn.TextColor3 = Palette.Text
    wBtn.BorderSizePixel = 0
    wBtn.MouseButton1Click:Connect(function()
        getgenv().SelectedWeaponType = wepName
        WepDropdownBtn.Text = "Senjata: " .. wepName .. " ▼"
        WepListScroll.Visible = false
        WepDropdownContainer.Size = UDim2.new(1, -5, 0, 35)
    end)
end

WepDropdownBtn.MouseButton1Click:Connect(function()
    WepListScroll.Visible = not WepListScroll.Visible
    WepDropdownContainer.Size = WepListScroll.Visible and UDim2.new(1, -5, 0, 95) or UDim2.new(1, -5, 0, 35)
end)

createToggle("Auto Farm (All-in-One)", MainTab, function(state) 
    ...
    getgenv().AutoVoteEnabled = state
    getgenv().AutoClickMainEnabled = state
    getgenv().AntiVoidEnabled = state
    getgenv().NoCdReloadEnabled = state
...
createToggle("No CD Reload (Bypass Cooldown)", MainTab, function(state) getgenv().NoCdReloadEnabled = state end)
createToggle("Anti Void (Teleport Player/Atas)", MainTab, function(state) getgenv().AntiVoidEnabled = state end)
createToggle("Auto Replay (Klik Main jika Mati)", MainTab, function(state) getgenv().AutoClickMainEnabled = state end)
createToggle("Hit Lewat Dinding / Radius Jauh (Reach)", MainTab, function(state) getgenv().HitboxExpanderEnabled = state end)
createToggle("Auto Isi Vote (System Choice)", MainTab, function(state) getgenv().AutoVoteEnabled = state end)
...
createToggle("Player ESP (Wallhack & Health Bar)", VisualsTab, function(state)
...
createToggle("Auto Aim (Head Lock)", PlayerTab, function(state) getgenv().AutoAimEnabled = state end)
createToggle("Auto Shot (Gunakan Tool)", PlayerTab, function(state) getgenv().AutoShotEnabled = state end)
createToggle("Auto Run (Cari Musuh)", PlayerTab, function(state) getgenv().AutoRunEnabled = state end)

createSectionHeader("Misc Tools", PlayerTab)
createButton("Leave Match (Kembali ke Lobby)", PlayerTab, function()
    local rs = game:GetService("ReplicatedStorage")
    if rs:FindFirstChild("ReturnToLobby") then
        rs.ReturnToLobby:FireServer()
    elseif rs:FindFirstChild("LobbyEvent") then
        rs.LobbyEvent:FireServer()
    else
...
if TabButtons["Main"].Icon:IsA("ImageLabel") and not string.find(TabButtons["Main"].Icon.Image, "rbxthumb") then 
    TabButtons["Main"].Icon.ImageColor3 = Palette.Button 
elseif TabButtons["Main"].Icon:IsA("TextLabel") then 
    TabButtons["Main"].Icon.TextColor3 = Palette.Button 
end

-- [[ 4. LOGIKA & LOOPS (SYSTEM PINTAR) ]]
...
-- LOGIKA BARU V1.0.2: Auto-Click "MAIN" Saat Mati (DI-UPGRADE)
task.spawn(function()
    while task.wait(0.5) do
        if getgenv().AutoClickMainEnabled then
            if not LocalPlayer.Character or (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") and LocalPlayer.Character.Humanoid.Health <= 0) then
                for _, gui in pairs(LocalPlayer.PlayerGui:GetDescendants()) do
                    if (gui:IsA("TextButton") or gui:IsA("ImageButton")) and gui.Visible then
                        local btnName = string.lower(gui.Name)
                        local btnText = gui:IsA("TextButton") and string.lower(gui.Text) or ""
                        
                        -- Target spesifik tombol dengan nama atau teks "MAIN"
                        if string.match(btnName, "main") or btnText == "main" or string.match(btnName, "play") or btnText == "play" then
                            pcall(function()
                                -- Cara 1: Menggunakan fungsi koneksi bawaan executor (jika ada)
                                if getconnections then
                                    for _, conn in pairs(getconnections(gui.MouseButton1Click)) do conn:Fire() end
                                    for _, conn in pairs(getconnections(gui.MouseButton1Down)) do conn:Fire() end
                                    for _, conn in pairs(getconnections(gui.MouseButton1Up)) do conn:Fire() end
                                    for _, conn in pairs(getconnections(gui.Activated)) do conn:Fire() end
                                end
                                
                                -- Cara 2: Memaksa simulasi kursor dan klik fisik menggunakan VirtualInputManager
                                local absPos = gui.AbsolutePosition
                                local absSize = gui.AbsoluteSize
                                local centerX = absPos.X + (absSize.X / 2)
                                local centerY = absPos.Y + (absSize.Y / 2) + 36 -- Compensate untuk topbar inset
...
table.insert(ActiveConnections, RunService.RenderStepped:Connect(function()
    ...
    -- LOGIKA BARU V1.0.2: Anti Void (Deteksi Jatuh & Teleport)
    if getgenv().AntiVoidEnabled and root then
        -- Jika pemain jatuh ke kedalaman lebih dari -50 (ketinggian void normal roblox sebelum mati)
        if root.Position.Y < -50 then
            ...
            if targetP and targetP.Character and targetP.Character:FindFirstChild("HumanoidRootPart") then
                -- Teleport ke atas kepala target
                root.CFrame = targetP.Character.HumanoidRootPart.CFrame * CFrame.new(0, 5, 0)
                root.Velocity = Vector3.new(0, 0, 0)
            else
                -- Jika tak ada pemain lain, teleport balik ke ketinggian yang aman
    ...
    -- LOGIKA BARU V1.0.2: No CD Reload (DI-UPGRADE)
    if getgenv().NoCdReloadEnabled and char then
        local tool = char:FindFirstChildOfClass("Tool")
        if tool then
            for _, v in pairs(tool:GetDescendants()) do
                if v:IsA("NumberValue") or v:IsA("IntValue") then
                    local nameLow = string.lower(v.Name)
                    -- Menghapus semua cooldown (mencari cd, reload, delay)
                    if string.match(nameLow, "reload") or string.match(nameLow, "cooldown") or string.match(nameLow, "wait") or string.match(nameLow, "delay") or string.match(nameLow, "cd") then
                        v.Value = 0
                    -- Mengunci amunisi pada value yang besar (mencari ammo, clip, mag)
                    elseif string.match(nameLow, "ammo") or string.match(nameLow, "clip") or string.match(nameLow, "mag") or string.match(nameLow, "peluru") then
                        if v.Value < 50 then v.Value = 999 end 
                    end
                end
            end
        end
    ...
    if getgenv().AutoAimEnabled or getgenv().AutoFarmEnabled then
    ...
    if (getgenv().AutoShotEnabled or getgenv().AutoFarmEnabled) and char then
    ...
    if (getgenv().AutoRunEnabled or getgenv().AutoFarmEnabled) and hum and root then
        local target = getClosestPlayer()
        if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
            local dist = (root.Position - target.Character.HumanoidRootPart.Position).Magnitude
            if getgenv().SelectedWeaponType == "Pistol/Gun" and dist < 20 then
                -- Berhenti jika cukup dekat
            else
    ...
    if getgenv().HitboxExpanderEnabled then
        ...
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                if p.Character.HumanoidRootPart.Size.X > 5 then
    ...
    if getgenv().EspEnabled then
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("Head") and p.Character:FindFirstChild("Humanoid") then
                local eColor = isEnemy(p) and Color3.fromRGB(255, 50, 50) or Color3.fromRGB(50, 150, 255)
                
                if not EspHighlights[p] then
                    local hl = Instance.new("Highlight", CoreGui)
                    hl.FillColor = eColor; hl.OutlineColor = Color3.fromRGB(255, 255, 255)
                    hl.FillTransparency = 0.6; hl.Adornee = p.Character
                    EspHighlights[p] = hl
                else 
                    EspHighlights[p].Adornee = p.Character
                    EspHighlights[p].FillColor = eColor
                end
                ...
                if not EspGuis[p] then
                    ...
                    local nameL = Instance.new("TextLabel", bgui)
                    nameL.Size = UDim2.new(1, 0, 0.5, 0)
                    nameL.BackgroundTransparency = 1
                    nameL.Text = p.Name
                    nameL.TextColor3 = eColor
                    ...
                    hpBg.Position = UDim2.new(0.1, 0, 0.6, 0)
                    ...
                    EspGuis[p] = {Gui = bgui, Bar = hpBar}
                else
                    EspGuis[p].Gui.Adornee = p.Character.Head
                    local hpPerc = p.Character.Humanoid.Health / p.Character.Humanoid.MaxHealth
                    EspGuis[p].Bar.Size = UDim2.new(hpPerc, 0, 1, 0)
                    if hpPerc < 0.3 then EspGuis[p].Bar.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
                    elseif hpPerc < 0.6 then EspGuis[p].Bar.BackgroundColor3 = Color3.fromRGB(255, 255, 0)
                    else EspGuis[p].Bar.BackgroundColor3 = Color3.fromRGB(0, 255, 0) end
                end
            end
        end
    end
...
table.insert(ActiveConnections, Players.PlayerRemoving:Connect(function(player)
    ...
    if EspGuis[player] then EspGuis[player].Gui:Destroy(); EspGuis[player] = nil end
end))
